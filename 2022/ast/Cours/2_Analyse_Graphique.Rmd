...
```{r,fig.height=5}
plot(UKDriverDeaths,
	 main = "Nombre de conducteurs de voitures tués au RU",
	 ylab = "Nombre")
```
...
autoplot(UKDriverDeaths) +
	labs(title = "Nombre de conducteurs de voitures tués au RU",
		 y = "Nombre")
```
...
```{r,fig.height=5}
autoplot(nottem) + 
	ggtitle("Températures moyennes au Château de Nottingham") +
...
library(patchwork)
(autoplot(co2) + 
		labs(title = expression("Atmospheric concentration of CO"[2]),
			 y = "ppm")  + 
		autoplot(window(sunspots, start = 1900, end = 1930)) +
		labs(title = "Nombre mensuel de taches solaires", y = "Nombre")
) /
	(autoplot(austres) +
	 	labs(title = "Nombre de résidents australiens", y = "milliers") +
...
p2 = autoplot(ldeaths) +
	labs(title = "2. Nombre de décès de maladies pulmonaires au Royaume-Uni", y = "Nombre")
...
\end{cases}
$$
Voir https://aqlt-formation-ast.netlify.app/backcasttransform.pdf
