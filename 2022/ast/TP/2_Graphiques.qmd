...
ggAcf(hsales, 12*7)
```
On augmentant le nombre de lag on repère plus facilement les cycles longs (environ 8 ans)
:::
...
# La méthode Guerrero donne lambda = -0.3. Le graphique est proche de celui du passage au log : la recommandation est de garder la transformation en log, plus explicable
...
# Aucune transformation n'est nécessaire. La méthode Guerrero donne lambda = 0.0 (log) sans que cela change les résultats
BoxCox.lambda(co2)
...
# La transformation en log semble suffisante, légèrement trop forte.
